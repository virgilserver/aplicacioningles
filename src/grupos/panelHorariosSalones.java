 ...
 * @author Luis
 */
public class panelHorariosSalones extends javax.swing.JPanel {
...
//    
 ...
 ArrayList<hora> horas ;
    ...
    TreeMap<String, Integer> idSalones = null;
    /** ArrayList<hora> horas ;
     * Creates new form panelHorariosSalones
     */
    public panelHorariosSalones() {
  ...
  private void llenarScrollPanelNiveles() {
    ...
    private void comboSalonesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_comboSalonesActionPerformed
        ...
        if (bool) {
            ...
            try {
                int id = idSalones.get(comboSalones.getSelectedItem().toString());
    ...
    private void llenarComboSalones() {
     ...
     try {
         ResultSet rs = ConexionSQLServer.vistaIdSalon();
         
         while (rs.next()) {
             String salon = rs.getString(1);
             int idSalon = rs.getInt(2); 
             
             comboSalones.addItem(salon);
              idSalones.put(salon, idSalon);
         }
     } catch (SQLException ex) {
         Logger.getLogger(panelHorariosSalones.class.getName()).log(Level.SEVERE, null, ex);
     }     
